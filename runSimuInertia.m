...
%% I=0.10
%"Healthy Controls - HC"
pert_x_HC10 = zeros(nbForce, nbSim, 8, round((timeStab + timeFree ) / dt)); % 8 = nb of state variables
pert_xest_HC10 = zeros(nbForce, nbSim, 8, round((timeStab + timeFree  ) / dt));
...
for f = 1:3
    for i = 1:nbSim
        delayError = 1; % Delay error in percentage
        [x, u, x_est,xy,L_HC] = simulation(timeStab, delayError, delayError,timeFree, f, x0,delta,.10);
...
for f = 1:3
    for i = 1:nbSim
        delayError=.7; % Delay is underestimated : Delay used is 70% of the actual delay
        [x, u, x_est,xy, L_ET10] = simulation(timeStab, delayError,delayError, timeFree, f, x0,delta,0.10);
...
%% I = 0.20
%"Healthy Controls - HC"
pert_x_HC20 = zeros(nbForce, nbSim, 8, round((timeStab + timeFree ) / dt)); % 8 = nb of state variables
pert_xest_HC20 = zeros(nbForce, nbSim, 8, round((timeStab + timeFree  ) / dt));
...
for f = 1:3
    for i = 1:nbSim
        delayError = 1; % Delay error in percentage
        [x, u, x_est,xy,L_HC20] = simulation(timeStab, delayError, delayError,timeFree, f, x0,delta,.2);
...
for f = 1:3
    for i = 1:nbSim
        delayError=.7; % Delay is underestimated : Delay used is 70% of the actual delay
        [x, u, x_est,xy, L_ET20] = simulation(timeStab, delayError,delayError, timeFree, f, x0,delta,0.2);
